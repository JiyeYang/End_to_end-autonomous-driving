...
# 配置
csv_file = 'E:/Udacity 自驾车数据集/udacity_Ch3_001/steering_new.csv'
image_dir = 'E:/Udacity 自驾车数据集/udacity_Ch3_001/center_new'
batch_size = 128
num_epochs = 50
...
def train_and_validate(start_epoch, num_epochs, train_loader, val_loader, model, criterion, optimizer, scheduler,
    ...
    # 绘制损失曲线
    plt.plot(range(start_epoch, num_epochs), train_losses, label='Train Loss')
    plt.plot(range(start_epoch, num_epochs), val_losses, label='Val Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
...
if __name__ == '__main__':
    ...
    optimizer = optim.Adam(model.parameters(), lr=0.06)  # 将初始学习率改为0.008
    scheduler = lr_scheduler.StepLR(optimizer, step_size=15, gamma=0.9)  # 使用学习率调度器 StepLR: 每step_size个 epoch 后，将学习率调整为当前学习率的gamma倍 调整num_epochs/step_size次
...
if __name__ == '__main__':
    ...
    criterion = nn.MSELoss()  # 使用均方误差作为损失函数
    optimizer = optim.Adam(model.parameters(), lr=0.06)  # 将初始学习率改为0.008
    scheduler = lr_scheduler.StepLR(optimizer, step_size=15, gamma=0.9)  # 使用学习率调度器
